...
describe Atm do
  ...
  it 'reject withdraw if pin is wrong' do
    ...
    expect(subject.withdraw(50, '1111', account)).to eq expected_output
  end
  ...
  it "reject if account is not active" do
    allow(account).to receive(:account_status).and_return(:inactive)
    expected_output = { status: false, message: 'card not active', date: Date.today }
    expect(subject.withdraw(50, '1234', account)).to eq expected_output
  end
